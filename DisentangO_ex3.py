...
class VAE(nn.Module):
    def __init__(self, latent_dim, nparam):
        super().__init__()
        self.nparam = nparam
...
class DisentanglementNO(nn.Module):
    ...
    def forward(self, x, task_idx):
        ...
        z, lifting_params_recons = self.vae(lifting_params_encoded)
...
def main(iseed, learning_rate, gamma, wd, latent_dim):
    t0 = default_timer()
    ...
    if normalizer == 'UnitGaussianNormalizer':
        learning_rate_pt, gamma_pt, wd_pt = 6e-2, 0.5, 3e-4
    ...
    ################################################################
    load_dir = './data_new/heteroHGO_load.mat'
    disp1_dir = './data_new/heteroHGO_disp_1s.mat'
    disp2_dir = './data_new/heteroHGO_disp_2s.mat'
    disp3_dir = './data_new/heteroHGO_disp_3s.mat'
    disp4_dir = './data_new/heteroHGO_disp_4s.mat'
    reader = MatReader(disp1_dir)
    sol1_total = reader.read_field('disp').view(100, sample_per_task, S, S, 2)
    reader = MatReader(disp2_dir)
    sol2_total = reader.read_field('disp').view(100, sample_per_task, S, S, 2)
    reader = MatReader(disp3_dir)
    sol3_total = reader.read_field('disp').view(100, sample_per_task, S, S, 2)
    reader = MatReader(disp4_dir)
    sol4_total = reader.read_field('disp').view(100, sample_per_task, S, S, 2)
    sol_total = torch.cat((sol1_total, sol2_total, sol3_total, sol4_total), dim=0)

    reader = MatReader(load_dir)
    f_total = reader.read_field('load').view(1, sample_per_task, S, S, 2).repeat(task_num_total, 1, 1, 1, 1)

    sol_train = sol_total[:train_task_num, ...].reshape(ntrain, S, S, 2).permute(0, 3, 1, 2).reshape(ntrain, -1, S)
    f_train = f_total[:train_task_num, ...].reshape(ntrain, S, S, 2)

    sol_valid = sol_total[-(valid_task_num + test_task_num):-test_task_num, ...].reshape(
        nvalid, S, S, 2).permute(0, 3, 1, 2).reshape(nvalid, -1, S)
    f_valid = f_total[-(valid_task_num + test_task_num):-test_task_num, ...].reshape(nvalid, S, S, 2)

    sol_test = sol_total[-test_task_num:, ...].reshape(ntest, S, S, 2).permute(0, 3, 1, 2).reshape(ntest, -1, S)
    f_test = f_total[-test_task_num:, ...].reshape(ntest, S, S, 2)
    ...
    # train and eval.
    ################################################################
    pretrain_model_dir = './metaNO_ex3_train32_task_%d_test/shal2deep_ifno' % train_task_num
    base_dir = './DNO_ex3_freezeMetaNO%s_l%d' % (model_dir_suffix, latent_dim)
    os.makedirs(base_dir, exist_ok=True)

    res_dir = './res_DNO_ex3_freezeMetaNO%s' % model_dir_suffix
    os.makedirs(res_dir, exist_ok=True)
    res_file = "%s/DNO_ex3_freezeMetaNO_n%d%s_l%d.txt" % (
        res_dir, ntrain, model_dir_suffix, latent_dim)
    if not os.path.isfile(res_file):
        f = open(res_file, "w")
        f.write(f'ntrain, seed, lr, gamma, wd, train_lowest, train_best, train_abs_data, train_abs_recons, train_kl, '
                f'train_l2_data, train_l2_recons, valid_best, valid_abs_data, valid_abs_recons, valid_kl, '
                f'valid_l2_data, valid_l2_recons, test, test_abs_data, test_abs_recons, test_kl, test_l2_data, '
                f'test_l2_recons, best_epoch, time (hrs)\n')
        f.close()
    ...
    train_abs_data_best = train_abs_recons_best = train_l2_data_best = train_l2_recons_best = train_kl_best = 1e8
    valid_abs_data_best = valid_abs_recons_best = valid_l2_data_best = valid_l2_recons_best = valid_kl_best = 1e8
    for ep in range(epochs):
        optimizer = scheduler(optimizer, LR_schedule(learning_rate, ep, step_size, gamma))
        model.train()
        t1 = default_timer()
        train_l2 = 0.0
        train_abs_data = train_l2_data = train_abs_recons = train_l2_recons = train_kl = 0.0
        for minibatch in zip(*train_loader):
            ...
            for task_idx, (x, y) in enumerate(minibatch):
                ...
                loss += loss_data + loss_recons + loss_kl
            loss.backward()
            optimizer.step()
            train_l2 += loss.item()
        ...
        if train_l2 < train_loss_best:
            model.eval()
            valid_l2 = 0.0
            valid_abs_data = valid_abs_recons = valid_l2_data = valid_l2_recons = valid_kl = 0.0
            with torch.no_grad():
                for minibatch in zip(*valid_loader):
                    for task_idx, (x, y) in enumerate(minibatch):
                        ...
                        valid_l2 += loss_data + loss_recons + loss_kl
            ...
            if valid_l2 < valid_loss_best:
                ...
                print(f'>> ep [{(ep + 1): >{len(str(epochs))}d}/{epochs}], runtime: {(t2 - t1):.1f}s, '
                      ...
                      f'(drk: {train_abs_data:.3f}/{train_abs_recons:.3f}/{train_kl:.3f}), '
                      f'valid err: {valid_l2:.4f} '
                      f'({valid_abs_data:.3f}/{valid_abs_recons:.3f}/{valid_kl:.3f})')
    ...
    model.eval()
    test_l2 = 0.0
    test_abs_data = test_abs_recons = test_l2_data = test_l2_recons = test_kl = 0.0
    with torch.no_grad():
        for minibatch in zip(*test_loader):
            for task_idx, (x, y) in enumerate(minibatch):
                ...
                test_l2 += loss_data + loss_recons + loss_kl
    test_abs_data /= ntest
        ...
        for lr in lrs:
            for gamma in gammas:
                for wd in wds:
                    ...
                    print("-" * 100)
                    main(iseed, lr, gamma, wd, latent_dim)
