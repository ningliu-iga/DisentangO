...
def main(random_seed, learning_rate, gamma, wd, learning_rate_pt, gamma_pt, wd_pt):
    ...
    nprevalid = valid_task_num * ntotal_per_task

    ntrain = test_task_num * ntrain_per_task
    ntest = test_task_num * ntest_per_task
    ...
    f_test_mixed = f_indis_mixed[-ntrain - ntest - nprevalid:-nprevalid],
    y_test_mixed = y_indis_mixed[-ntrain - ntest - nprevalid:-nprevalid]
    ...
    # train and eval.
    ################################################################
    pretrain_model_dir = './metaNO_HGO_neumann_train_task_%d_valid/shal2deep_ifno' % train_task_num
    base_dir = './metaNO_HGO_neumann_train_task_%d_test/shal2deep_ifno' % train_task_num
    os.makedirs(base_dir, exist_ok=True)

    res_dir = './res_metaNO_HGO_neumann_test_task_%d' % train_task_num
    os.makedirs(res_dir, exist_ok=True)
    res_file = "%s/metaNO_HGO_n%d_test.txt" % (res_dir, ntrain)
    ...
    model_filename = '%s/ifno_depth%d_plr%.1e_pgamma%.1f_pwd%.1e_lr%.1e_gamma%.1f_wd%.1e.ckpt' % (base_dir,
                                                                                                  ...
                                                                                                  gamma,
                                                                                                  wd)
    pretrain_model_filename = '%s/ifno_depth%d_plr%.1e_pgamma%.1f_pwd%.1e_lr%.1e_gamma%.1f_wd%.1e.ckpt' % (
        pretrain_model_dir, nb, learning_rate_pt, gamma_pt, wd_pt, learning_rate, gamma, wd)

    if not os.path.isfile(pretrain_model_filename):
        print('*** WARNING: pretrained meta-valid model not found. Exiting..')
        return

    print(">> Loading pretrained meta-valid model from %s" % pretrain_model_filename)
    ...
    for i in range(test_task_num):
        pretrained_dict['fc0.%d.weight' % (train_task_num + valid_task_num + i)] = tmp_weight
        pretrained_dict['fc0.%d.bias' % (train_task_num + valid_task_num + i)] = tmp_bias
    model.load_state_dict(pretrained_dict)

    # only train lifting layer and freeze others
    all_lifting_parameters = []
    for i in range(test_task_num):
        all_lifting_parameters += list(model.fc0[train_task_num + valid_task_num + i].parameters())
    ...
    for ep in range(epochs):
        ...
        for minibatch in zip(*train_loader):
            ...
            for task_idx, (x, y) in enumerate(minibatch):
                ...
                out = model(x, train_task_num + valid_task_num + task_idx).reshape(this_batch_size, 2 * s, s)
